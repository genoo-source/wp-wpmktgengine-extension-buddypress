...
add_action('wpmktengine_init', function($repositarySettings, $api, $cache){
    ...
    add_action('bp_forums_new_topic', function($topic_id) use ($api){
        // Get user
    ...
    add_action('bp_forums_new_post', function($topic_id) use ($api){
        // Get user
        $user = wp_get_current_user();
        $topic = get_post($topic_id);
    ...
    add_action('groups_create_group', function($groupid, $member, $group) use($api){
        // Get who created it
    ...
    add_action('groups_join_group', function($group_id, $user_id) use ($api){
        $user = get_userdata($user_id);
        $name = new BP_Groups_Group($group_id, array());
        $name = $name->name;
    }, 10, 2 );
    ...
    add_action('groups_leave_group', function($group_id, $user_id) use ($api){
        $user = get_userdata($user_id);
        $name = new BP_Groups_Group($group_id, array());
        $name = $name->name;
    }, 10, 2 );
